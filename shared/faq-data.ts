// Single source of truth for FAQ content. Imported by both the React FAQ page
// (client/src/pages/faq.tsx) and the crawler-friendly /faq-plain server route.

export interface FaqItem { q: string; a: string }
export interface FaqGroup { groupKey: string; heading: string; items: FaqItem[] }
...
export const faqGroups: FaqGroup[] = [
  {
    ...
    items: [
      ...
      {
        q: "What is the Aave Hub?",
        a: "The Aave Hub (Pro tier) lets you supply, borrow, repay, and withdraw on Aave v3 across Ethereum, Base, Arbitrum, and Polygon — without leaving CryptoOwnBank. Connect your MetaMask or WalletConnect wallet, pick a chain, and you see live supply and borrow APYs straight from the Aave v3 Pool contract (no third-party indexer, no lag). Your supplied positions and borrowed positions are read on-chain and refresh every minute. Click Supply, Withdraw, Borrow, or Repay and we build the transaction — your wallet pops up to sign it. CryptoOwnBank never holds your keys, never holds your funds, and never takes a fee on your Aave activity. Supply flows include the standard ERC-20 approve step (sign once per token amount). Borrowing shows your live health factor with a color-coded warning when it's getting close to liquidation. The page is a UI to a public, open-source protocol — we are not affiliated with Aave Companies or the Aave DAO, and we don't provide financial advice. Borrowing carries real liquidation risk: if your collateral value falls or your debt grows, Aave will sell part of your collateral plus a penalty. Most safe borrowers keep their health factor above 1.8. v1 supports USDC, USDT, DAI, WETH, WBTC, cbBTC, and WMATIC where each is available; more chains and assets are easy to add.",
      },
      {
        q: "Does CryptoOwnBank control my money or make decisions for me?",
        a: "No. We're built around one simple idea: we bring everything to the table, and you're the only one who says yes. Here's what that means in practice — (1) We're the table: everything you need to set up, review, and act on is gathered in one place. (2) We're the explainer: we tell you in plain words what each step does before you do it. (3) We're the checklist and tracker: we make sure nothing that needs doing gets missed, and you can follow along after. (4) You're the only one who says yes: nothing happens until you approve it with your own wallet. We never hold your money, your keys, or your recovery phrase, and we never decide who you're allowed to deal with. We're not trying to hook into everyone and everything — the power stays with you and the technology, not with us. That's the difference between a tool that works for you and a company that works on you.",
      },
      {
  ...
  },
  {
    ...
    items: [
      {
        q: "How do I create a wallet on CryptoOwnBank?",
        a: "Go to <a href=\"/wallet/create\"><strong>/wallet/create</strong></a> and pick one of three modes. <strong>Generate</strong> — we create a fresh 12-word seed phrase in your browser, you write it down, verify two of the words, and you're done in about 90 seconds. <strong>Import</strong> — paste an existing 12, 15, 18, 21, or 24-word BIP-39 seed phrase from Xaman, MetaMask, Ledger, or any other compatible wallet, and your XRPL address is derived instantly so everything you already own shows up here without moving. <strong>Roll your own entropy</strong> — for sovereignty-minded members who don't want to trust any software random number generator: type in the result of physical dice rolls or coin flips and we hash it down to a seed in your browser. Whichever mode you pick, the keys are generated on your device and never leave it. We never see, store, or transmit your seed phrase. We literally cannot recover it if you lose it — that's the entire point of being your own bank.",
      },
      {
        q: "Does this wallet only work on XRP, or on other chains too?",
        a: "Other chains too — that's the magic of BIP-39. The 12-word seed phrase you write down is a <em>universal</em> master key. The same words, fed into different standardized derivation paths, produce valid addresses on roughly 22+ blockchains. On the success screen at <a href=\"/wallet/create\"><strong>/wallet/create</strong></a> we now show all of them, computed in your browser from the same seed: XRP Ledger, Ethereum (and every EVM chain — Polygon, Arbitrum, Optimism, Base, BNB, Avalanche, Linea, zkSync, Gnosis, Fantom, Scroll), Bitcoin, Litecoin, Dogecoin, Bitcoin Cash, TRON, Cosmos Hub (plus Osmosis, Juno, Akash via the same key), Solana, and Stellar. One click saves all of them to your CryptoOwnBank wallet list. You walked into the page wanting an XRP address and walked out owning an address on most of the public chains that matter. None of those addresses cost us anything to generate, because we generated nothing — your browser did the math.",
      },
      {
        q: "Why does the EVM section say \"Ethereum & all EVM chains\" with one address?",
        a: "Because every EVM-compatible chain — Ethereum, Polygon, Arbitrum, Optimism, Base, BNB Chain, Avalanche C-Chain, Linea, zkSync, Gnosis, Fantom, Scroll, and dozens more — uses the same address format derived from the same path (m/44'/60'/0'/0/0). The 0x... address you see is valid on all of them simultaneously. If someone sends you USDC on Polygon to that address, it lands. If someone sends you ETH on Arbitrum to that address, it lands too. The address itself doesn't care which chain — what determines \"which chain\" is the RPC endpoint your wallet is pointed at when it scans for balances. This is why MetaMask and Rabby let you flip between networks without changing addresses. One key, twelve+ chains, zero extra ceremony.",
      },
      {
        q: "Why doesn't the address list include Cardano (or Polkadot, Algorand, Aptos, Sui, Near, Tezos, Hedera)?",
        a: "Because those chains use non-standard derivation schemes that don't fit the BIP-39 → BIP-44 path model we use for the others. The math is still there — your 12 words still control those chains mathematically — but each of those projects invented their own derivation specs (Cardano uses CIP-1852, Polkadot uses the sr25519 curve, Algorand has its own 25-word mnemonic standard, NEAR uses named accounts, Aptos and Sui use ed25519 with their own account derivation, Tezos uses ed25519 with its own encoding, Hedera requires on-network account creation with a fee). To support them properly we'd have to ship per-chain wallet libraries with their own bugs and edge cases. We chose to support the 22 chains where the standard is settled and ship now, rather than wait. <strong>The right move if you want those chains:</strong> install the official wallet for that ecosystem (Yoroi / Daedalus for Cardano, Polkadot.js / Talisman for Polkadot, Pera or Defly for Algorand, Petra for Aptos, Suiet for Sui, NEAR Wallet for NEAR, Temple for Tezos, HashPack for Hedera), select \"Restore wallet,\" and paste the same 12 words you wrote down. The addresses will appear immediately. Your seed phrase is the master key — what changes is which app knows how to read it.",
      },
      {
        q: "If I lose CryptoOwnBank, do I lose access to all 22 of those addresses?",
        a: "No — and this is the whole point. Your 12 words are the wallet. CryptoOwnBank is just one of many viewers on top of those words. If we vanish overnight: open any standards-compliant wallet (Xaman for XRP, MetaMask or Rabby for EVM, Sparrow or Electrum for Bitcoin, Phantom or Solflare for Solana, Keplr for Cosmos, Lobstr or Freighter for Stellar, etc.), select \"Restore from seed phrase,\" paste your 12 words, and every one of those addresses appears with its current balance and full transaction history pulled directly from the blockchain. We were never in the loop. The annual <a href=\"/sovereignty\"><strong>sovereignty drill</strong></a> exists precisely to make sure you've tested this — once a year, restore your seed in a fresh wallet app, confirm the addresses match, then close it. That single 5-minute test is the difference between believing you own your crypto and proving it.",
      },
      {
        q: "What does \"non-custodial\" actually mean when I create a wallet here?",
        a: "It means the cryptographic keys to your wallet are generated and held only on your device. CryptoOwnBank's servers receive only your <em>public</em> address — the same string anyone can look up on a blockchain explorer — so we can show you balances, prepare unsigned transactions, and display the wallet in your account. We never receive, store, or transmit your seed phrase or private key. The <code>/wallet/create</code> page even has server-side guards that refuse to save any field that looks like a BIP-39 phrase, an XRPL family seed (sXXX format), or a hex private key — if you accidentally paste a secret into the label field, the server rejects it and warns you. You can verify all of this yourself: open your browser's developer tools, click into the Network tab, and watch the wallet creation flow. The only thing that goes to our server is <code>{ chain, address, label }</code> — never the seed. Our code is also AGPL-3.0 licensed, so you don't have to take our word for it.",
      },
      {
        q: "What happens if I lose my seed phrase?",
        a: "You lose access to that wallet forever. There is no \"forgot password\" button, no support team that can reset it, no recovery email. This is the trade-off of true self-custody — the same trade-off that applies to physical cash in a jar or gold buried in a yard: nobody can take it from you because nobody else can access it, and that includes us. This is why the wallet creation flow forces you through a backup ceremony before the wallet is saved: reveal the words, acknowledge in writing that you've written them down, then type back two random words to prove you really did. We recommend writing them on the printable backup template, storing them on a steel backup plate (search Cryptosteel or Billfodl — no affiliate), keeping them in a fireproof safe, splitting between a bank safety deposit box and a trusted location, or any combination. The one thing that does NOT count as backup: a screenshot or a note in your phone or your email. Those get hacked.",
      },
      {
        q: "Can I bring my existing Xaman, Ledger, or MetaMask wallet into CryptoOwnBank?",
        a: "Yes — and you don't have to move any crypto to do it. The Import mode at <a href=\"/wallet/create\"><strong>/wallet/create</strong></a> accepts any BIP-39 seed phrase (12, 15, 18, 21, or 24 words) from any compatible wallet. When you paste it, the seed is validated in your browser, the XRPL address is derived using the standard BIP-44 derivation path (m/44'/144'/0'/0/0 — the same one Xaman uses), and only the public address is saved to your CryptoOwnBank account. Your existing wallet keeps working exactly as before — both Xaman and CryptoOwnBank are now just two different windows looking at the same address on the XRP Ledger. For Ledger hardware wallets, the connection still happens through Xaman as before (CryptoOwnBank doesn't yet support direct USB/Bluetooth Ledger pairing in the create-wallet flow — that's on the roadmap). For MetaMask and other EVM wallets, see the connection flow under EVM Swap and Cross-Chain Swap; in-browser EVM wallet creation is also on the roadmap.",
      },
      {
        q: "Can I buy XRP with a debit or credit card directly on CryptoOwnBank yet?",
        a: "Not yet, but it's coming. We've applied for Stripe Crypto Onramp access — the same payment infrastructure powering most of the internet's checkout flows — so that once your wallet is created at <a href=\"/wallet/create\"><strong>/wallet/create</strong></a>, you'll be able to buy XRP with a card and have it delivered straight to your new wallet, no exchange in the middle, no custody, no transfer-out step. Stripe handles the fiat conversion under their licenses; we never touch your money, only pass them your address. Approval is currently pending. In the meantime, you can receive XRP from any source — another wallet, a friend, or an exchange withdrawal — to your new address and it will appear automatically. The XRP Ledger does require any new account to hold 1+ XRP as an activation reserve (this is a ledger-level rule, not us); we don't activate it for you because that would be custody.",
      },
      {
        q: "Why do I have to write down 12 words instead of just using a username and password?",
        a: "Because your wallet IS those 12 words — they're not a backup of it, they literally are it. A username and password authenticate you to a company's database (Coinbase, your bank, Google). Your account exists because the company says it does, and they can freeze, close, or hand it over on request. A seed phrase is mathematically the wallet — anyone in the world holding those words has the same control you do, and no company, government, or court can revoke it. That's the entire reason crypto exists. The price of that sovereignty is responsibility: you're now the bank, which means you're also the security guard. We make the ceremony as gentle as we can — readable type, blur-then-reveal, verification, a downloadable backup template, and printable storage advice — but we won't pretend the responsibility isn't real, because pretending is what custodial exchanges do, and it ends badly every cycle.",
      },
      {
        q: "Do I need to create a new wallet here if I already have one elsewhere?",
        a: "No. If you already have a wallet you trust — Xaman, Ledger, ELLIPAL, any BIP-39 source — use Import mode at <a href=\"/wallet/create\"><strong>/wallet/create</strong></a> instead of Generate. Pasting your existing seed phrase lets CryptoOwnBank see and manage the same wallet without creating a second one, so all your existing XRP, RLUSD, trustlines, Soil vault positions, and transaction history appear in our interface immediately. The seed phrase is validated and processed entirely in your browser — it never leaves your device. Creating a second new wallet is only useful if you want a separate one for trading or experimenting while keeping your main holdings elsewhere.",
      },
      {
      ...
      {
        q: "What is the Wallet Security Center, and why should I check my token approvals?",
        a: "Every time you use a DeFi app — a swap, a marketplace, a lending pool — you grant it an \"approval\": permission to move a specific token out of your wallet. Most apps ask for an <em>unlimited</em> approval so you don't have to re-approve every time. The problem is those permissions don't expire. They sit there for years, and if that app (or a copy of it) is ever hacked or turns malicious, the approval is a standing door into your wallet. The <a href=\"/wallet-security\"><strong>Wallet Security Center</strong></a> connects to your EVM wallet (MetaMask or WalletConnect), reads the blockchain to find every open approval you've ever granted across Ethereum, Polygon, Base, Arbitrum (plus Optimism, Avalanche, and BNB Chain on request), flags the unlimited ones, and shows how much is actually at risk. When you want to close a door, click <strong>Revoke</strong> — that's a normal blockchain transaction you sign yourself in your own wallet (it costs a small network fee, called gas). The scan is read-only and we never touch your funds or keys. A couple of honest limits: it covers standard tokens (ERC-20), not NFTs yet, and because we use free public network connections, a very busy wallet may come back as a \"partial\" scan — just run that network again on its own for a fuller view. It's a Premium feature, alongside our other EVM tools.",
      },
      {
        q: "Can I borrow against my crypto without selling it — and without giving up custody?",
        a: "Yes — through <a href=\"/aave\"><strong>Aave</strong></a>, one of the most established non-custodial lending protocols, reachable right inside CryptoOwnBank. You supply a crypto asset you already own as collateral, then borrow against it (often a stablecoin) so you get cash to use without selling your position or triggering a taxable sale. You can also repay and withdraw at any time. Everything is signed in your own wallet — Aave is a set of smart contracts, not a company holding your money, and CryptoOwnBank never holds, lends, or touches your funds. We're just the window: \"access Aave from your wallet,\" never \"borrow from CryptoOwnBank.\" The one thing to take seriously is liquidation risk — if your collateral drops in value and your loan gets too close to the limit, the protocol can sell some of your collateral automatically, so borrow conservatively and keep an eye on your health factor. You can read the plain-language overview at <a href=\"/defi-borrowing\"><strong>/defi-borrowing</strong></a> and click straight through to the live Aave hub. This is a tool, not financial advice.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "The Legacy Plan helps your family recover your crypto after you pass away — or if you become incapacitated. Choose <strong>$29/year</strong>, <strong>$99 for 5 years</strong>, or <strong>$499 once for life</strong>; Member for Life is also included with Pro. It's a dead-man switch: you check in on a schedule you choose (weekly, biweekly, monthly, or quarterly). If you miss a check-in, a grace period begins. During the grace period, your secondary contact (spouse, attorney, etc.) is notified to verify your status. If the grace period expires with no response, your beneficiaries receive the wallet recovery instructions you set up — where your hardware wallet is stored, where the seed phrase backup is located, how to access a CypheRock 2-of-5 Shamir setup, etc.<br/><br/><strong>What we NEVER store:</strong> seed phrases, private keys, passwords, or PINs. We only store YOUR instructions about <em>where</em> those things are — 'steel plate in the fireproof safe' or 'Card 1 with attorney, Card 3 in bank safe deposit box.' Everything is encrypted at rest and delivered only when the switch triggers.<br/><br/><strong>Readiness honesty check:</strong> confirmed beneficiaries — including 3/3 — show that your people are ready, not that every asset is covered. Do not treat the plan as fully ready until every intended wallet has recovery instructions and any SLIP-39 setup has enough correctly assigned and tested shards.",
      },
      {
      ...
      {
        q: "What is the Sovereignty Recovery Kit, and how does it work with the Legacy Plan?",
        a: "The <strong>Sovereignty Recovery Kit</strong> is a printable, self-contained HTML document you can download for free from <code>/sovereignty-kit</code> — every tier, including Free. It lists your <strong>public</strong> wallet addresses grouped by chain (never seed phrases or private keys — we don't have those), plus chain-by-chain restore guidance: which wallets to install for BTC, XRPL, Stellar, EVM chains, and Monero, how to import a seed, how to verify balances, how to store the phrase on paper or metal, and an annual sovereignty drill checklist so you prove to yourself once a year that you can still get to your assets without us. It's the document you use to <em>verify your own sovereignty</em> while you're alive.<br/><br/><strong>How it feeds into the Legacy Plan:</strong> if you have a Legacy Plan, the same chain-by-chain restore guidance is bundled automatically into your Survivability Export as an appendix titled 'How to actually restore the assets,' reframed for the beneficiary audience. Your family gets one complete document — beneficiary instructions, device locations, encrypted vaults, AND restore guidance — not two files to track down in a stressful moment. Update one source, both documents stay current. The kit on <code>/sovereignty-kit</code> is for you; the bundled appendix in the Legacy Plan export is for the people you leave it to.",
      },
      {
      ...
      },
      {
        ...
        a: "Yes — and you have three ways to pay for it: <strong>$29/year</strong>, <strong>$99 for 5 years</strong>, or <strong>$499 once for life</strong>. The Pro tier ($99/mo or $799/yr) includes Legacy Plan free. Whichever you choose, the check-in timer keeps running, your beneficiary instructions stay encrypted and ready, and the dead-man switch stays active during the paid term. If a term expires, the plan is paused (not deleted) — your data is preserved so you can resume anytime. The lifetime option removes renewal risk, but your inheritance coverage is only complete after every intended wallet and any required SLIP-39 shards are fully set up.",
      },
      {
      ...
      {
        q: "What is House Tier for BTC, ETH, SOL, XRP, and RLUSD?",
        a: "We call those five our <strong>House Tier</strong> because they are our preferred treasury coins: BTC, ETH, and SOL are established assets we're comfortable holding, while XRP and RLUSD are our home-chain assets. House Tier is a preferred-coin designation, not an extra percentage benefit. <strong>Every supported cryptocurrency receives the same 10% on-chain discount</strong> on subscriptions, add-ons, and Legacy Lifetime purchases.<br/><br/><strong>Founder Badge:</strong> if you buy <strong>Legacy Plan Lifetime</strong> in any House Tier coin, your account gets a permanent visible Founding Member badge showing which coin you paid in and the month. It's our way of recognizing members who support the platform with one of its preferred treasury coins.",
      },
      {
        q: "Can I switch from a term plan to lifetime later?",
        a: "Yes. If you're already on the $29/year or $99/5-year Legacy Plan, you can switch to the $499 lifetime option later. Your existing Legacy Plan data — beneficiaries, encrypted vaults, SLIP-39 shards, check-in history, and settings — is preserved; only the billing term changes. We don't pro-rate or refund the current term.",
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      {
        q: "Which wallet is best — and do I need more than one?",
        a: "The right question isn't \"which wallet is best,\" it's \"what job is this wallet doing for me?\" Different wallets are good at different jobs. A <strong>cold wallet</strong> (hardware, air-gapped) is your vault — it holds the bulk of your wealth offline, with the strongest protection against hacks, app bugs, and everyday mistakes. You bring it out rarely and carefully, the way you'd open a safety deposit box, and you don't connect it to every new site. A <strong>working wallet</strong> (a mobile or hardware wallet you plug into apps) is for participation — lending, trading, and using features like XLS-66 lending. It's closer to a debit card than a vault: convenient and connected, but not where you keep everything you own. A <strong>hot wallet like Xaman</strong> sits in the middle — built to connect smoothly to the XRPL, understand new transaction types quickly, and show you clear, human-readable signing screens. Plenty of people use more than one wallet on purpose: the smart move isn't to find one device that magically does everything, it's to give each wallet a job and stick to it.",
      },
      {
        q: "Should I keep everything on one wallet?",
        a: "We don't recommend it. Think about it the way you already think about money in the bank: you don't keep your life savings on the card you shop online with — you keep most of it in a secure account and top up the card with what you need. If the card is ever compromised, the damage is limited to what was on it. Apply that exact pattern to your crypto: keep the bulk of your XRP and long-term holdings on a <strong>cold wallet you almost never connect</strong>; when you want to lend through XLS-66, move only what you intend to use into a more connected <strong>working wallet</strong> (for example Xaman, or a hardware wallet wired for our site); use that wallet to sign, repay, and withdraw; then sweep profits and unused funds back to cold storage when you're done. Cold wallets are your vault; Xaman and compatible hardware are your debit cards. Every time you connect a wallet to a website you're making a trade — more security usually means less convenience, and more convenience means more exposure. Separating your savings from your spending lets you have both.",
      },
      {
        q: "How does CryptoOwnBank support using different wallets for different jobs?",
        a: "We're built around this layered approach, not against it. We treat <strong>vault storage</strong> and <strong>lending participation</strong> as two separate roles, even if you use the same brand of wallet for both. We're explicit about <strong>clear signing vs. blind signing</strong>, so you always know when your hardware wallet is showing you the real transaction details and when it isn't — see the <a href=\"/signing-options\" class=\"text-[#00A4E4] hover:underline\">Signing Options</a> page and its wallet compatibility list. And our policies are designed so you can keep most of your assets in the most conservative setup you're comfortable with while still participating in XLS-66 lending — without dragging your entire cold stack into every new connection. We don't ask you to choose between maximum security and full utility. We help you separate your savings from your spending, so you can have both.",
      },
    ],
  },
  {
