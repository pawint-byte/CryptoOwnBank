...
`);
  ...
  // ?lang= localizes <html lang> + title/description/canonical, and P0 FAQ Q&A via localizeFaqGroups; remaining FAQ stays English.
  {
    ...
    app.get("/faq-plain", (req, res) => {
      try {
        const lang = pickLang(req);
        const meta = getSeoMeta(lang);
        const title =
          lang === "en"
            ? "CryptoOwnBank FAQ (Plain) — Crawler-friendly"
            : `${meta.title} — FAQ`;
        const description =
          lang === "en"
            ? "Full FAQ for CryptoOwnBank rendered as plain HTML for AI fetchers and search crawlers. Interactive version at /faq."
            : meta.description;
        const canonical =
          lang === "en"
