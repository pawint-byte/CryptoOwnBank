...
- **XLS-65/66 Native Vaults & Lending** (`/xls66-lending`): Complete infrastructure for XRPL Single Asset Vaults (XLS-65) and Lending Protocol (XLS-66). **Amendment auto-detection**: Backend queries XRPL mainnet `feature` command every 10 min to check if `SingleAssetVault` and `LendingProtocol` amendments are enabled — no manual flags needed. `XLS66_LIVE` env var available as manual override. **Live vault discovery**: Queries `ledger_data` with `type: "vault"` every 5 min, filters blocklisted vaults, returns raw on-chain data (no endorsements). **Schema**: `xls66_vaults`, `xls66_positions` (user deposits with shares, yield tracking, auto-reinvest), `xls66_loan_offers` (collateralized lending), `xls66_vault_blocklist` (admin-managed). **Backend**: Full CRUD routes at `/api/xls66/*` with Pro tier gating, yield calculator (simple + compounding), trustline setup via Xumm, VaultDeposit/VaultWithdraw signing with Xaman payload polling, amendment status endpoint, admin blocklist management (`/api/xls66/admin/blocklist` GET/POST/DELETE). **Frontend**: Tabbed page — Vaults (on-ledger discovery cards with disclaimer, XRPL explorer links, no endorsement badges), Calculator (APR/APY with compounding), Trustlines (one-click RLUSD/USD/EUR), Borrowing Hub (external DeFi protocol directory: Aave/Compound/Spark/Venus — users visit those platforms directly). Deposit modal with amount input and Xaman signature polling (3-sec interval, shows signed/rejected status). Amendment banner shows 3 states. **Admin**: Vault blocklist management page at `/admin/vault-blocklist`. Files: `client/src/pages/xls66-lending.tsx`, `client/src/pages/admin-vault-blocklist.tsx`, `shared/schema.ts`, `server/storage.ts`, `server/routes.ts`.

- **Native Staking Guide** (`/native-staking`): Educational page covering protocol-level staking across 5 chains: XRP (XLS-65 vaults, pending), ADA (Cardano delegation, live), ATOM (Cosmos delegation, live), DOT (Polkadot NPoS, live), SOL (Solana delegation, live). Shows APR, lock-up periods, risk levels, mechanism type for all tiers. Premium/Pro members get full step-by-step guides, wallet recommendations, validator selection tips, and risk considerations. Free members see overview + APR comparison with upgrade CTA. Public route (no login required) for SEO. Files: `client/src/pages/native-staking.tsx`.
