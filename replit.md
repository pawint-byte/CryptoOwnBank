...
- [ ] **Automated Tax-Loss Harvesting** — Added to public roadmap (Horizon). Scan positions for unrealized losses, surface harvest opportunities with estimated tax savings. Two modes: (1) "harvest & rebuy immediately" (current loophole — wash-sale rule does NOT apply to spot crypto as of Mar 2026, crypto = property not securities), (2) "conservative swap" with similar-but-not-identical assets (XRP → payment token, RLUSD → USDC). Member executes trades themselves. Disclaimers: not tax advice, rules may change, Congress has proposed closing the gap.
- [ ] **Borrow Against Yield Positions** — Added to public roadmap (Horizon). Interface to external lending protocols ONLY — CryptoOwnBank never holds collateral or sets terms. Depends on XLS-65/66 activation for XRPL-native lending. Language: "borrow through the protocol via our interface" not "borrow on our platform." Regulatory caution: facilitating lending = potential money transmission.
- [ ] **AI Portfolio Assistant** — Pay-as-you-go via OpenAI API. ~$0.01–$0.03 per conversation. No monthly minimum. Start with GPT-4o-mini (~1/10th cost), upgrade to GPT-4o when needed (single line change). Sign up at platform.openai.com, add key as Replit secret `OPENAI_API_KEY`. Instant activation, no approval. Make it Premium/Pro exclusive (Free=0, Premium=50/mo, Pro=unlimited). Must enforce "not financial advice" in system prompt. OpenAI chosen over Anthropic for speed, numerical reasoning, and system prompt adherence.
