...
- **XLS-66 Native Lending** (`/xls66-lending`): Complete infrastructure for XRPL Single Asset Vaults (XLS-66 amendment). Feature-flagged via `XLS66_LIVE` env var — all UI renders but deposits/withdrawals/loans are blocked until amendment activates on mainnet. **Schema**: `xls66_vaults` (vault registry with APR, capacity, lock periods, risk levels), `xls66_positions` (user deposits with shares, yield tracking, auto-reinvest), `xls66_loan_offers` (collateralized lending with LTV, liquidation thresholds). **Backend**: Full CRUD routes at `/api/xls66/*` with Pro tier gating, yield calculator endpoint (simple + daily/weekly/monthly compounding), trustline setup via Xumm, VaultDeposit/VaultWithdraw transaction signing, amendment status check, auto-reinvest toggle. Xumm payload whitelist expanded for XLS-66 fields (VaultID, SharesIn, SharesOut, MPTokenIssuanceID, LoanAmount, CollateralAmount, etc.). **Frontend**: Tabbed page with Vaults (card grid with APR, utilization, one-click deposit), Calculator (amount/APR/duration/compounding with APY conversion), Trustlines (one-click RLUSD/USD/EUR setup), Borrowing Hub (Aave/Compound/Spark/Venus directory). Positions summary with auto-reinvest toggles. Educational guide explaining SAV mechanics, MPT shares, and risk disclosure. Pro tier required; upgrade prompt for lower tiers. Files: `client/src/pages/xls66-lending.tsx`, `shared/schema.ts`, `server/storage.ts`, `server/routes.ts`.
