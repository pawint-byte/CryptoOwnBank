...
- [ ] **AI Portfolio Assistant** — Pay-as-you-go via OpenAI or Anthropic API. ~$0.01–$0.03 per conversation. No monthly minimum. Sign up at platform.openai.com (key starts with `sk-...`) or console.anthropic.com (key starts with `sk-ant-...`). Add as Replit secret `OPENAI_API_KEY` or `ANTHROPIC_API_KEY`. Instant activation, no approval. Make it Premium/Pro exclusive with usage caps (Free=0, Premium=50/mo, Pro=unlimited). Must enforce "not financial advice" in system prompt.
