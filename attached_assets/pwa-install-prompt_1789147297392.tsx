...
/** Delay first show so marketing and first-session UX are not interrupted. */
const SHOW_DELAY_MS = 50_000;
...
export function PWAInstallPrompt() {
  ...
  const onMarketing = isPublicMarketingPath(location);
  ...
  useEffect(() => {
    ...
    let pendingNative: BeforeInstallPromptEvent | null = null;
    let pendingIos: Mode = null;

    const maybeShow = () => {
      if (cancelled || !delayElapsed) return;
      if (pendingNative) {
        setDeferredPrompt(pendingNative);
        setMode("native");
      } else if (pendingIos) {
        setMode(pendingIos);
      }
    ...
    const handler = (e: Event) => {
      e.preventDefault();
      pendingNative = e as BeforeInstallPromptEvent;
      maybeShow();
    };
    window.addEventListener("beforeinstallprompt", handler);

    if (isIos) {
      pendingIos = isIosSafari ? "ios-safari" : "ios-chrome";
    }
  ...
  if (!mode || onMarketing) return null;
      ...
      <Card className="p-3 shadow-2xl border-primary/30 bg-background/95 backdrop-blur-md">
        <div className="flex items-start gap-3">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-10 w-10 -mt-1 -mr-1 shrink-0"
            onClick={dismiss}
