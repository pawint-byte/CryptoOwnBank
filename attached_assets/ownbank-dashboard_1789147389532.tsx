            ...
            <div>
              ...
              {rlusdBalance > 0 && (
                <div className="mt-0.5">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-0.5" data-testid="text-soil-yield-honesty">
                    Third-party vaults; rates vary, not guaranteed. We never hold keys.
                  </p>
                </div>
              )}
            </div>
                    ...
                    {soilSummary.vaults.map((v) => (
                      <div key={v.address} className="rounded-lg border bg-card p-2.5" data-testid={`card-vault-${v.name.toLowerCase()}`}>
                        ...
                        <div className="flex items-center justify-between pt-1.5 border-t">
                          <p className="text-[9px] text-muted-foreground flex-1 mr-2">
                            Interest accrues daily but is not added to your principal. Withdraw and re-deposit to earn on a larger balance.
                          </p>
                          <Link href="/ownbank/withdraw">
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-5 text-[9px] px-2 gap-0.5 shrink-0"
                              data-testid={`button-withdraw-${v.name.toLowerCase()}`}
                            >
                              Withdraw
                            </Button>
                          </Link>
                  ...
                  onClick={async () => {
                    try {
                      await navigator.share({
                        title: "CryptoOwnBank — Self-custody RLUSD tools",
                        text: "Explore third-party RLUSD yield vaults (rates vary; not guaranteed). Self-custody — we never hold keys.",
