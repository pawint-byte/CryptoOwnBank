...
- **XRPL:** No new mainnet amendments changed everyday account/AMM/clawback/lending behavior in ~30 days. Context worth knowing: Ripple published a post-quantum-readiness roadmap (PQC migration targeted no later than 2028 — someday self-custody users may need to migrate funds to PQ-secure accounts); XRPL AMM is described as structurally flash-loan-resistant (a security positive). No chain halts, exploits, or key-compromise events.
...
## SHIPPED — Wallet-strategy copy: "Your wallet is a role in your strategy" (Added & completed 2026-05-31)

Founder supplied copy framing wallets as *jobs* rather than "which wallet is best" — cold wallet = vault (survival), working/utility wallet = participation (XLS-66 lending), Xaman = the in-the-middle working wallet, plus the "debit card" mental model (keep the bulk cold, move only working funds, sweep back) and the clear-vs-blind-signing tradeoff. Asked to put it on the FAQ + site when ready. Tone kept calm and plain-language to match the site ethos.

- **On-page block** (`client/src/pages/signing-options.tsx`): a new "Your wallet is a role in your strategy" Card placed right after the intro card and before the wallet test panel — three role cards (Cold = vault, Working = debit card, Hot/Xaman = in the middle) plus a "debit-card rule" summary. Content-only; uses existing Card components, brand accent `#00A4E4`, new lucide icons (Layers/Snowflake/CreditCard).
- **FAQ** (`shared/faq-data.ts`): 3 new Q&A entries in the **Secure Crypto Storage Options** group — "Which wallet is best — and do I need more than one?", "Should I keep everything on one wallet?", and "How does CryptoOwnBank support using different wallets for different jobs?" (the last links to `/signing-options`). Answers use the same safe static HTML (`<strong>`, internal `<a>`) the rest of the FAQ uses.
- No logic changes. tsc clean on changed files; `/faq` and `/signing-options` both 200; architect review passed (optional future follow-ups noted: sanitize/whitelist FAQ HTML, de-dupe wallet-strategy language across FAQ groups).

---

## SHIPPED — Browser hardware-wallet connectors: Trezor + Keystone (Added & completed 2026-05-31)

Founder owns a Cypherock and wanted to connect + test real hardware wallets now, ahead of XLS-66 vault support going live, so members aren't blocked later. Scope decided: build all buildable browser connectors now, and surface an honest request to lift the security filter so Cypherock can follow.

- **What shipped:** a "Test your hardware wallet" panel on `/signing-options` (`client/src/components/wallet-test-panel.tsx`) where a member connects a device and sees their XRP address — proving the wiring works without moving any funds.
  - **Trezor** (`client/src/lib/trezor-connector.ts`): loaded from Trezor's **hosted** connect script (`https://connect.trezor.io/9/trezor-connect.js`), NOT the npm package. `connectTrezor()` reads the XRP address; `signWithTrezor(tx)` signs an XRP payment. Connects over USB on Chrome/Edge desktop via Trezor's own secure pop-up.
  - **Keystone** (`client/src/lib/keystone-connector.ts`): air-gapped QR signing via `@keystonehq/keystone-sdk` + `@ngraveio/bc-ur` (installed cleanly). Reads the account QR with the camera (`@keystonehq/animated-qr` `AnimatedQRScanner`, lazy-loaded), with helpers to build a sign-request QR and parse the signature QR.
  - **Capability matrix** (`client/src/lib/signer-capabilities.ts`): Trezor + Keystone now `integrated: true` with **Payment = clear** (works today). TrustSet and XLS-66 vault deposits kept **unsupported** — honest, since those paths aren't wired and Trezor's XRP API is payment-only. New `cypherock` entry (`integrated: false`) explains the security-filter block. Arculus and the "other" group (SafePal/Ellipal) reframed as **maker-app-only** — by design they only sign in their maker's own phone app and cannot be driven from any website. "Coming soon" lane description made neutral to cover both cases.

- **BLOCKER — Cypherock (and the Trezor npm package) cannot be installed here:** both depend on `protobufjs`, which this project's package firewall blocks package-wide (403 on every version tried). Trezor worked around it via the hosted script; Cypherock has no equivalent and is deferred. **Action needed from founder:** ask Replit support / the workspace package policy to allow-list `protobufjs` (and the `@cypherock/sdk-*` packages). Once lifted, Cypherock connects over USB just like Trezor and we switch it on.

- **Architect review applied before ship:** downgraded TrustSet/vault fidelity to honest `unsupported` (was overstated as clear/blind); fixed a Trezor loader bug where a failed script load couldn't be retried without a page reload; made the Keystone camera-error path stop the scanner and show a clear message.
...
- [2026-05-30] Self-correcting transfers — once an address is recognized as the member's own wallet (any non-"manual" chain) or a known vault (SOIL/DOPPLER), past AND future outgoing "sells" to it auto-reclassify to non-taxable transfers, with phantom gains cleared and lots restored. Added `counterpartyAddress` column to transactions (shared/schema.ts) so the destination is recorded and old rows can be matched retroactively. Both sync paths (backgroundWalletSync + POST /api/wallets/:id/sync) now persist it on new txns and backfill it on already-imported rows missing it. New shared service server/services/transfer-reconciliation.ts holds the (moved) idempotent clearGainEventsForSell + selfCorrectKnownTransfers(userId), called after add-wallet and at the end of both syncs. Matching is chain-aware (exact for case-sensitive chains, case-insensitive for EVM) and serialized per-user to avoid concurrent double-restore. Old held rows converge with a one-sync delay (next sync of the SENDING wallet backfills the destination, then self-correct flips it). Manual sells/swaps have no counterpartyAddress so are never auto-touched. Architect-reviewed: all three flagged issues fixed (manual sync path wired, chain-aware matching, per-user concurrency lock).
