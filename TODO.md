...
**Last updated: 2026-05-26 by main agent (Window-dressing pass: regrouped sidebar from 7 mixed groups [start/portfolio/market/ownbank/bridges/planning/learn, ~67 items] into 7 foundation-first destinations — Home, My Wallets, Execute, Back Up & Recover, Move Money, Plan & Protect, Learn & Decide. Pure UI change: same URLs, same backends, same page contents underneath. Chain switcher moved into Execute group and now filters Execute/Move/Protect items globally. Added one-time `<SovereigntyAcknowledgement>` card — 5 checkbox statements covering lost-seed-irrecoverable, anyone-with-words-can-steal, we-never-ask, no-screenshots, big-amounts-need-big-protection. Persists via `useUserData("sovereignty_acknowledged_at", null)` — no schema change. Injected into wallet-create done step (with "create"/"import" intent variant), top of `/sovereignty` page for logged-in users, and as a gentle dismissible-by-acknowledging banner on Dashboard when user has wallets but hasn't acknowledged. No hard blocks anywhere per founder's explicit direction.)
