...
**Last updated: 2026-05-26 by main agent (Educational copy add: new "second honest distinction" section on `/sovereignty` covering the Bybit Feb 2025 incident, the verify-on-device discipline, and a narrowed honest claim about what our servers do/don't hold; new reusable `<VerifyOnDeviceNotice />` component dropped into ownbank-send and ownbank-transfer confirmation dialogs so members are reminded to read the destination address and amount on Xaman/Ledger's own screen before approving — never blind-sign. Tone calibrated per architect review: removed absolutes ("cannot be lied to" → "generally much harder to alter"), attributed Bybit causality to "published post-mortems," softened headline tone, narrowed server-seizure claim to acknowledge account metadata exists. Source frame for this work: ET Spotlight article on wallet signing environments — used the substance, skipped the vendor pitch.)
