...
- To log a verified test user in dev: `POST /api/auth/signup` then `UPDATE users SET email_verified = true` via `executeSql` (dev DB). Do NOT use the founder email `pawint@me.com` for test logins — risks corrupting the real account and leaking their addresses into committed screenshots.
