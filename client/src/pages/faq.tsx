  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "It depends on what type of cold wallet you have. For a hardware wallet (Ledger Nano X): Open Xaman on your phone → tap the account icon or go to Settings → Accounts → 'Add Account' → 'Import an existing account' → 'Pair with Ledger.' Make sure Bluetooth is enabled, your Ledger is unlocked with the XRP app open, and Xaman will detect it automatically. Your private keys stay on the Ledger — Xaman just acts as the interface. For Tangem or NFC-based cards: Same flow — Add Account → Import → choose 'Tangem Card' → tap the card to your phone when prompted. For ELLIPAL or SafePal (air-gapped): Add Account → Import → 'Read-only address' and paste your r-address. You'll sign transactions by scanning QR codes between your phone and the device. If you have a paper wallet or know your secret key (family seed or mnemonic): Add Account → Import → choose 'Secret Numbers,' 'Family Seed,' or 'Mnemonic' and enter it. Xaman encrypts it on-device and never sends it anywhere. Once imported, you can switch between accounts in Xaman by tapping the account name at the top. Each wallet signs its own transactions independently. For a full walkthrough, see our Setup Guide page.",
      },
      {
