  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "It depends on what type of cold wallet you have. <strong>For ELLIPAL or SafePal (air-gapped):</strong> Open Xaman → tap 'Import an existing account' → choose 'Read-only address' → paste your XRP r-address from your ELLIPAL app (go to the coin → tap Receive → copy the address) → tap Next → give it a name like 'ellipal-xrp' (pick a name that reminds you what it's for — like future XLS-65 lending) → done! You'll see your XRP balance right away. Important: Xaman and your ELLIPAL device are looking at the <em>same address</em> on the XRP Ledger — your XRP doesn't move. Xaman is just a window to view your balance and build transactions. When you need to sign (like a future XLS-65 lending transaction), Xaman shows a QR code on your phone → you scan it with your physical ELLIPAL device's camera → the ELLIPAL signs it offline → it displays a signed QR code on its screen → you scan that back with Xaman on your phone to submit. Your private keys never leave the ELLIPAL device. Note: you need the physical ELLIPAL hardware device for signing — the ELLIPAL app on the same phone as Xaman can't scan QR codes between apps. <strong>For a hardware wallet (Ledger Nano X):</strong> Open Xaman → 'Import an existing account' → 'Pair with Ledger.' Make sure Bluetooth is enabled, your Ledger is unlocked with the XRP app open, and Xaman will detect it automatically. Your private keys stay on the Ledger — Xaman just acts as the interface. <strong>For Tangem or NFC-based cards:</strong> Same flow — Import an existing account → choose 'Tangem Card' → tap the card to your phone when prompted. <strong>If you have a paper wallet or know your secret key</strong> (family seed or mnemonic): Import an existing account → choose 'Secret Numbers,' 'Family Seed,' or 'Mnemonic' and enter it. Xaman encrypts it on-device and never sends it anywhere. Once imported, you can switch between accounts in Xaman by tapping the account name at the top. Each wallet signs its own transactions independently.",
      },
      {
