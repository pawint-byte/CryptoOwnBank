...
const EXCHANGE_WITHDRAWAL_RESTRICTIONS: Record<string, Record<string, ExchangeWithdrawalInfo>> = {
  uphold: {
    ...
    SGB: { supported: false, note: "Uphold is delisting SGB after March 30, 2026 — sell or swap before then", workaround: "Swap SGB to XRP on Uphold before March 30 → withdraw XRP → re-buy SGB on Bifrost Wallet or a Songbird DEX if desired" },
...
const WALLET_STAKING_GUIDES: Record<string, Record<WalletBrand, WalletAction[]>> = {
  ...
  XDC: {
    ...
    ellipal: [
      ...
      { text: "Alternative: Send XDC to MetaMask (add XDC Network RPC: https://erpc.xinfin.network, Chain ID 50), then connect to XDC Web Wallet" },
    ...
    tronlink: [],
    metamask: [
      { text: "Add the XDC Network RPC to MetaMask (Network Name: XDC Mainnet, RPC: https://erpc.xinfin.network, Chain ID: 50, Symbol: XDC, Explorer: https://xdcscan.io)" },
