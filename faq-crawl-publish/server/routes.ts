...
`);
  ...
  // Crawler-friendly FAQ: Express owns GET /faq (and /faq-plain) with server-rendered
  // HTML + FAQPage JSON-LD so AI fetchers / search crawlers that don't execute JS
  // get full content. Self-canonical is https://cryptoownbank.com/faq(?lang=).
  // Note: full-document GET /faq no longer serves the React SPA shell (client/src/pages/faq.tsx
  // remains for in-app navigation / HMR, but Express wins over the SPA catch-all).
  // ?lang= localizes <html lang> + FAQ-specific title/description/canonical, and P0 FAQ
  // Q&A via localizeFaqGroups; remaining FAQ stays English.
  {
    const { localizeFaqGroups } = await import("@shared/faq-p0-i18n");
    const { pickLang } = await import("./seo-localize");
    type SeoLang = import("./seo-localize").SeoLang;
    const escapeHtml = (s: string) =>
      s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
    const stripHtml = (s: string) => s.replace(/<[^>]*>/g, "");

    // FAQ-specific SEO — do NOT reuse homepage getSeoMeta titles (those caused
    // "?lang=es" pages to show homepage meta + " — FAQ").
    const FAQ_SEO: Record<SeoLang, { title: string; description: string }> = {
      en: {
        title: "CryptoOwnBank FAQ — Self-Custody, Legacy Plan, Yield & Payments",
        description:
          "CryptoOwnBank FAQ: non-custodial self-custody, Legacy Plan inheritance, yield tracking via third-party protocols (not guaranteed), payments, wallets, and help topics.",
      },
      es: {
        title: "FAQ de CryptoOwnBank — Autocustodia, Plan Legado, Rendimiento y Pagos",
        description:
          "Preguntas frecuentes de CryptoOwnBank: autocustodia no custodial, Plan Legado, seguimiento de rendimiento vía protocolos de terceros (sin garantía), pagos, carteras y temas de ayuda.",
      },
      pt: {
        title: "FAQ CryptoOwnBank — Autocustódia, Plano Legado, Rendimento e Pagamentos",
        description:
          "Perguntas frequentes da CryptoOwnBank: autocustódia não custodial, Plano Legado, acompanhamento de rendimento via protocolos de terceiros (sem garantia), pagamentos, carteiras e tópicos de ajuda.",
      },
      fr: {
        title: "FAQ CryptoOwnBank — Autoconservation, Plan Héritage, Rendement et Paiements",
        description:
          "Foire aux questions CryptoOwnBank : non dépositaire, Plan Héritage, suivi du rendement via protocoles tiers (sans garantie), paiements, portefeuilles et sujets d'aide.",
      },
      tr: {
        title: "CryptoOwnBank SSS — Kendi Saklama, Miras Planı, Getiri ve Ödemeler",
        description:
          "CryptoOwnBank SSS: saklamasız kendi saklama, Miras Planı, üçüncü taraf protokoller üzerinden getiri takibi (garanti yok), ödemeler, cüzdanlar ve yardım konuları.",
      },
      hi: {
        title: "CryptoOwnBank FAQ — सेल्फ-कस्टडी, लिगेसी प्लान, यील्ड और भुगतान",
        description:
          "CryptoOwnBank FAQ: नॉन-कस्टोडियल सेल्फ-कस्टडी, लिगेसी प्लान, तृतीय-पक्ष प्रोटोकॉल के माध्यम से यील्ड ट्रैकिंग (कोई गारंटी नहीं), भुगतान, वॉलेट और सहायता विषय।",
      },
      zh: {
        title: "CryptoOwnBank 常见问题 — 自我保管、传承计划、收益与支付",
        description:
          "CryptoOwnBank常见问题：非托管自我保管、传承计划、通过第三方协议追踪收益（不保证收益）、支付、钱包与帮助主题。",
      },
    };

    type FaqHtmlOpts = {
      lang: SeoLang;
      /** "primary" = /faq; "mirror" = /faq-plain (canonical still points at /faq). */
      variant: "primary" | "mirror";
    };

    const buildFaqPlainHtml = ({ lang, variant }: FaqHtmlOpts): string => {
      const meta = FAQ_SEO[lang];
      const canonical =
        lang === "en"
      ...
      const intro =
        variant === "primary"
          ? `<p>Server-rendered FAQ for search crawlers and AI assistants that do not execute JavaScript. A machine-readable version is available at <a href="/faq.txt">/faq.txt</a>${lang !== "en" ? ` (<a href="/faq.txt?lang=${lang}">?lang=${lang}</a>)` : ""}.</p>`
          : `<p>This page is a plain-HTML mirror of <a href="${escapeHtml(canonical)}">/faq</a> for AI assistants and crawlers that do not execute JavaScript. A machine-readable version is available at <a href="/faq.txt">/faq.txt</a>.</p>`;
      return `<!DOCTYPE html>
<html lang="${lang}">
<head>
<meta charset="utf-8">
...
${sections}
    ...
    const sendFaqHtml = (req: express.Request, res: express.Response, variant: "primary" | "mirror") => {
      try {
        const lang = pickLang(req);
        const html = buildFaqPlainHtml({ lang, variant });
        res.header("Content-Type", "text/html; charset=utf-8");
        res.header("Cache-Control", "public, max-age=300");
        res.send(html);
      } catch (err) {
        console.error(variant === "primary" ? "[faq]" : "[faq-plain]", err);
        res.status(500).type("text/plain").send("FAQ plain render failed");
      }
    };

    // Express routing is case-insensitive by default, so /FAQ would otherwise hit
    // the /faq handler with a 200. Redirect non-canonical casing to lowercase /faq
    // (preserve query string) before registering the content handler.
    app.use((req, res, next) => {
      if (req.method !== "GET" && req.method !== "HEAD") return next();
      const pathOnly = (req.path || "").split("?")[0];
      if (/^\/faq$/i.test(pathOnly) && pathOnly !== "/faq") {
        const q = req.url.includes("?") ? req.url.slice(req.url.indexOf("?")) : "";
        return res.redirect(301, `/faq${q}`);
      }
      next();
    });

    // Primary crawler URL — registered before SPA catch-all in static.ts / vite.ts.
    app.get("/faq", (req, res) => sendFaqHtml(req, res, "primary"));

    // Mirror kept for existing links / sitemap; same body, canonical still → /faq.
    app.get("/faq-plain", (req, res) => sendFaqHtml(req, res, "mirror"));
    ...
    app.get("/faq.txt", (req, res) => {
      try {
        ...
        console.error("[faq.txt]", err);
        res.status(500).type("text/plain").send("FAQ txt render failed");
      }
    });
  }
  ...
  app.get("/sitemap.xml", (_req, res) => {
    try {
      ...
      const publicRoutes = [
        ...
        { path: "/features/legacy-plan", priority: "0.9", changefreq: "monthly" },
        { path: "/features/legacy-plan/split-delivery", priority: "0.8", changefreq: "monthly" },
        { path: "/features/portfolio-tracker", priority: "0.9", changefreq: "monthly" },
        { path: "/features/decentralized-trading", priority: "0.8", changefreq: "monthly" },
        { path: "/features/yield-tracking", priority: "0.8", changefreq: "monthly" },
        { path: "/features/global-remittance", priority: "0.8", changefreq: "monthly" },
        { path: "/solutions", priority: "0.8", changefreq: "monthly" },
        { path: "/solutions/family-estate-planning", priority: "0.9", changefreq: "monthly" },
        { path: "/solutions/self-custody-security", priority: "0.9", changefreq: "monthly" },
        { path: "/solutions/active-crypto-investors", priority: "0.8", changefreq: "monthly" },
        { path: "/learn", priority: "0.8", changefreq: "weekly" },
        { path: "/learn/guides", priority: "0.8", changefreq: "weekly" },
        { path: "/learn/glossary", priority: "0.8", changefreq: "weekly" },
        { path: "/security", priority: "0.9", changefreq: "monthly" },
        { path: "/security/non-custodial-architecture", priority: "0.9", changefreq: "monthly" },
        { path: "/security/legacy-plan-check-ins", priority: "0.8", changefreq: "monthly" },
        { path: "/security/faq", priority: "0.8", changefreq: "monthly" },
      ...
      // KEY paths only — locale variants for crawlers (?lang=). Skip /faq.txt.
      const localeKeyPaths = new Set([
        "/",
        "/features/legacy-plan",
        "/solutions/family-estate-planning",
        "/learn",
        "/faq",
        "/faq-plain",
      ]);
      const localeCodes = ["es", "pt", "fr", "tr", "hi", "zh"] as const;
      const today = new Date().toISOString().split("T")[0];
      const bumpPriority = (p: string, delta: number) => {
        const n = Math.max(0, Math.round((parseFloat(p) + delta) * 10) / 10);
        return n.toFixed(1);
      };
      ...
      for (const r of publicRoutes) {
        ...
        if (localeKeyPaths.has(r.path)) {
          const locPriority = bumpPriority(r.priority, -0.1);
          for (const lang of localeCodes) {
            urlEntries.push(
              `  <url>\n    <loc>${baseUrl}${r.path}?lang=${lang}</loc>\n    <lastmod>${today}</lastmod>\n    <changefreq>${r.changefreq}</changefreq>\n    <priority>${locPriority}</priority>\n  </url>`
            );
          }
        }
      }
      const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urlEntries.join("\n")}\n</urlset>`;
      // Public + cross-origin so crawlers / external fetchers aren't blocked by Helmet CORP=same-origin
